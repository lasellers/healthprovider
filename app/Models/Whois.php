    ...
    **/
    public function whoislookup($domain)
    {
        ...
        else if(preg_match("/^([-a-z0-9]{2,100}).([a-z.]{2,8})$/i",$domain))
        {
            ...
            {
                \App\CLI::error(" Error: No appropriate Whois server found for $tld for $domain domain");
                    return "";
            }
            ...
            if(!$server)
            {
                \App\CLI::error(" Error: No appropriate Whois server found for $domain domain");
                    return "";
            }
    ...
    */
    private function queryWhois($server,$domain)
    {
        ...
        try {
            ...
            $this->command->line(" ** whoislookup: $domain on $server cache_file=$cache_file");
            
            if(file_exists($cache_file))
            {
                \App\CLI::info(" <<< whois: [cached] $domain on $server reading $cache_file ...");
        ...
        try
        {
            ...
            if(!$fp)
            {
                \App\CLI::warning("Socket Error " . $errno . " - " . $errstr);
                return "";
            }
        ...
        /* */
        \App\CLI::error(" >>> whois: $domain on $server ...");
