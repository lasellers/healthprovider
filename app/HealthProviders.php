...
class HealthProviders {
	...
	/**
	 **/
	function __construct( $command = null, $io ) {
		$this->io = $io;
		...
		$this->scraper = new SimpleCurl( $io );
		$this->dom     = new \DOMDocument();
		$this->dom2    = new \DOMDocument();
		$this->whois   = new Whois( $io );
	}
	...
	public function get_data() {
		$this->io->text( " * Querying $this->name $this->address $this->phone" );
		...
		$domains        = [ $this->domain ];
		$this->home_url = "";
		if ( count( $domains ) > 0 ) //        foreach($domains as $url)
		{
			$url = $domains[0];
			$this->io->text( " Looking for additional about/contact pages on $url..." );
		...
		$this->io->text( "\t name=" . $this->name );
		$this->io->text( "\t domain=" . $this->domain );
		$this->io->text( "\t email=" . $this->email );
		$this->io->text( "\t url=" . $this->url );

		$this->io->text( "\t phone=" . $this->phone );
		$this->io->text( "\t found phone=" . $this->found_phone );
		$this->io->text( "\t address=" . $this->address );
		$this->io->text( "\t city=" . $this->city );
		$this->io->text( "\t state=" . $this->state );
		$this->io->text( "\t zip=" . $this->zip );

		$this->io->text( "\t home_url=" . $this->home_url );
		foreach ( $this->aux_urls as $tag => $url ) {
			$this->io->text( "\t $tag url=" . $url );
		}

		foreach ( $this->ordered_domains as $domain => $count ) {
			$this->io->text( "\t\t > domain=" . $domain . " (" . $count . ")" );
		}
		foreach ( $this->ordered_emails as $email => $count ) {
			$this->io->text( "\t\t\t > email=" . $email . " (" . $count . ")" );
		}
	...
	public function yellowpages( $name, $city, $state ) {
		$this->io->text( " ** yellowpages $name,$city,$state" );
		...
		if ( get_class( $resultList ) == 'DOMNodeList' ) {
			...
			foreach ( $resultList as $i => $result ) {
				...
				if ( get_class( $city ) == 'DOMNodeList' ) {
					if ( count( $city ) > 0 ) //            foreach ($city as $result)
					{
						$resukt             = $city[0];
			...
			foreach ( $resultList as $i => $result ) {
				...
				if ( get_class( $state ) == 'DOMNodeList' ) {
					if ( count( $state ) > 0 ) // foreach ($state as $result)
					{
			...
			foreach ( $resultList as $i => $result ) {
				...
				if ( get_class( $zip ) == 'DOMNodeList' ) {
					if ( count( $zip ) > 0 ) //  foreach ($zip as $result)
					{
	...
	public function yellowpages_detail_page( $url ) {
		$this->io->text( "" );
		$this->io->text( " ** yellowpages page=$url" );
...
// --------------------------------------------------------------------
	...
	public function bing() {
					...
					foreach ( $itemList as $item ) {
						...
						if ( get_class( $aList ) == 'DOMNodeList' ) {
							foreach ( $aList as $anode ) {
								...
								if ( substr( $url, 0, 4 ) == 'http' ) {
									$this->io->text( " * bing local check: $url" );
		...
		$this->io->text( " -- bing phone..." . $this->phone );
						...
						if ( get_class( $aList ) == 'DOMNodeList' ) {
							foreach ( $aList as $anode ) {
								...
								if ( substr( $url, 0, 4 ) == 'http' ) {
									...
									if ( count( $domains ) > 0 ) {
										$this->io->text( " * $max_urls bing phone check: $url" );
										$data = $this->crawl_page( $url );
		...
		$this->io->text( " -- bing city+zip... " . $this->city . " " . $this->zip );
					...
					foreach ( $itemList as $item ) {
						...
						if ( get_class( $aList ) == 'DOMNodeList' ) {
							foreach ( $aList as $anode ) {
								...
								if ( substr( $url, 0, 4 ) == 'http' ) {
									/* test: nhc 24 */
									$this->io->text( " * $max_urls bing city+zip check: $url" );
									$domains = $this->reject_domains( [ $url ] );
...
// --------------------------------------------------------------------
	 ...
	 * @return domain
	 **/
	public function find_contact( string $url, string $html ) {
		$this->io->text( " ** find_contact=$url" );
...
// --------------------------------------------------------------------
	 ...
	 * @return domain
	 **/
	private function find_links_to_other_page( string $url, string $html, $tags ) {
		$this->io->text( " ** find_links_to_other_page=$url" );
		...
		$this->xpath = new \DOMXpath( $this->dom );
		$nodeList    = $this->xpath->query( "//a" );
		if ( is_object( $nodeList ) && get_class( $nodeList ) == 'DOMNodeList' ) { /* test hhc 376 */
			foreach ( $nodeList as $node ) {
				$href = $node->getAttribute( 'href' );
...
// --------------------------------------------------------------------
	 ...
	 * @return domain
	 **/
	public function find_auxillary_pages( string $url, string $html ) {
		$this->io->text( " ** find_auxillary_pages=$url" );
	...
	private function find_auxillary_page( $url, $html, $tag ) {
		//$this->io->text(" ** find_auxillary_page[$tag]=$url");
		if ( $url == "" | is_object( $html ) || is_array( $html ) || $html == "" ) {
			return "";
		}
		...
		if ( is_object( $nodeList ) && get_class( $nodeList ) == 'DOMNodeList' ) {
			...
			foreach ( $nodeList as $node ) {
				...
				$i = stripos( $href, $tag );
				if ( $tag == substr( $name, 0, strlen( $tag ) ) || ! ( $i === false ) ) {
					$this->io->text( "\t\t\t #### find_auxillary_page $tag \t $href" );
					if ( $href != '' ) {
						$page_url = "";
						$domain   = $this->host_from_url( $url );
...
// --------------------------------------------------------------------
	...
	/**
	 **/
	public function get_csv( string $url, string $zip_file, string $output_folder, string $csv_file ) {
		/* */
		$this->io->text( "get_csv()" );
		$this->io->text( " url=$url" );
		$this->io->text( " zip_file=$zip_file" );
		$this->io->text( " output_folder=$output_folder" );
		$this->io->text( " csv_file=$csv_file" );
		...
		if ( ! file_exists( $zip_file ) || filesize( $zip_file ) == 0 ) {
			...
			$this->io->note( "\t\t > Download $url size=" . filesize( $zip_file ) );
		}
		...
		if ( $res === true ) {
			...
			$this->io->note( " * Unzipped $zip_file size=" . filesize( $zip_file ) );
		} else {
			$this->io->error( " * Failed to unzipped $zip_file size=" . filesize( $zip_file ) );
		}

		$this->io->note( " * Extracted to output_folder ($csv_file)" );
	}
		...
		while ( ( $line = fgets( $handle ) ) !== false ) {
			...
			if ( $line_index == 0 ) {
				...
				if ( ! isset( $lookup[ $id ] ) ) {
					...
					$lookup[ $id ] ++;
					$this->io->error( "Dup $id (" . $lookup[ $id ] . ")" );
				}
			}
