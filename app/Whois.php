...
class Whois
{
    ...
    function __construct($io, $command = null)
    {
        $this->io=$io;
    ...
    public function whoislookup($domain)
    {
        ...
        } else if (preg_match("/^([-a-z0-9]{2,100}).([a-z.]{2,8})$/i", $domain)) {
            $domain_parts = explode(".", $domain);
            $tld = strtolower(array_pop($domain_parts));

            if (!isset($this->WHOIS_SERVERS[$tld])) {
                $this->io->error(" Error: No appropriate Whois server found for $tld for $domain domain");
                return "";
            }

            $server = $this->WHOIS_SERVERS[$tld][0];
            if (!$server) {
                $this->io->error(" Error: No appropriate Whois server found for $domain domain");
                return "";
            }

            $out = $this->queryWhois($server, $domain);
     ...
     */
    private function queryWhois($server, string $domain)
    {
        /* */
        //if(\Config::get("whois.cache")===true)
        //{
        try {
            $cache_file = storage_path() . "/cache/whois.$domain.txt";
           $this->io->text(" ** whoislookup: $domain on $server cache_file=$cache_file");

            if (file_exists($cache_file)) {
                $this->io->note(" <<< whois: [cached] $domain on $server reading $cache_file ...");
        ...
        /* */
        if (\Config::get('whois.cache_only')) return null;

        /* */
       $this->io->text("fsockopen $server ...");

        try {
            $fp = @fsockopen($server, 43, $errno, $errstr, 20);
            if (!$fp) {
                $this->io->warning("Socket Error " . $errno . " - " . $errstr);
                return "";
            }
        ...
        /* */
        $this->io->note(" >>> whois: $domain on $server ...");
