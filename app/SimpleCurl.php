...
class SimpleCurl
{
    ...
    */
    ...
    public function __construct($io, $config = null)
    {
        $this->io = $io;
    ...
    public function download_file($url, $local_file)
    {
        ...
        $this->io->error(" >>> download_file: url=$url local_file=$local_file");
        ...
        $this->io->error(" >>> download_file: url=$url file time=$filetime size=" . filesize($local_file));
    ...
    public function get_file($url)
    {
        ...
        try {
            $this->io->error(" <<< get_file: fetching... $url");

            $data = curl_exec($curl);
        } catch (Exception $e) {
            $this->io->error(" error: " . curl_error($curl));
        ...
        $this->io->comment("get_file: url=$url size=" . strlen($data));
    ...
    public function get_url($url, $cache = null, $options = null)
    {
        ...
        if ($cache != null) {
            try {
                ...
                if (is_array($options) && isset($options['headers'])) {
                    ...
                    $this->io->text("  get_url with header: cache_file $cache_file  cache_header_file=$cache_header_file ...");
                    if (file_exists($cache_header_file) && file_exists($cache_file)) {
                        $this->io->note(" <<< get_url: [cached] reading url=$url cache_file=$cache_file cache_header_file=$cache_header_file ...");
                    ...
                    $this->io->text("  get_url: cache_file $cache_file ...");
                    if (file_exists($cache_file)) {
                        if (filesize($cache_file) > $this->max_raw_size) return null;
                        if (filesize($cache_file) > $this->min_raw_size) {
                            $this->io->note(" <<< get_url: [cached] reading url=$url cache_file $cache_file ... size=" . filesize($cache_file));
        ...
        /* */
        try {
            $this->io->note(" <<< get_url: fetching... $url");
            $response = curl_exec($curl);
        } catch (Exception $e) {
            $this->io->error(" error(1): " . curl_error($curl));
            return ['error' => 'curl_exec failure: ' . $e->getMessage()];
        }
        /* */
        if (curl_errno($curl)) {
            ...
            $this->io->text(" # total_time:  $total_time");
            ...
            $this->io->error(" error(2): " . curl_error($curl));
        ...
        /* */
        $this->io->text(" # total_time:  $total_time");
        $this->io->text(" # namelookup_time:  $namelookup_time");
        $this->io->text(" # connect_time:  $connect_time");
        $this->io->text(" # pretransfer_time:  $pretransfer_time");
        $this->io->text(" # starttransfer_time:  $starttransfer_time");

        if ($total_time > $this->timeout - 1) {
            $this->io->text("");
            $this->io->text("");
            $this->io->text(" *** slow transfer ***");
            $this->io->text("");
            $this->io->text("");
            return "";
        }
        ...
        $this->io->note(" >>> get_url: writing $url size=" . strlen($body));

        /* */
        $this->sleep_between_calls();

        if (is_array($options) && isset($options['headers'])) {
            return [$header, $body, $options['headers']];
        } else {
            return $body;
        }
        /* */
    ...
    public function download_file_if_modified($url, $file)
    {
        ...
        if ($lt < $rt) {
            if ($this->verbose)
                $this->io->text("Get file=$file url=$url");
            ...
            if (!curl_errno($curl)) {
                ...
                $this->info = $info;
                if ($this->verbose)
                    $this->io->text('Took ' . $info['total_time'] . ' seconds to send a request to ' . $info['url']);
                if ($this->verbose)
                    $this->io->text(" CURL >>> size=" . $info['size_download'] . "");
            }
        ...
        if ($this->verbose)
            $this->io->text("Skipped file=$file url=$url");
        return false;
    }

    // --------------------------------------------------------------------
    public function post_form(string $url, $fields, $cache = null, $options = null)
    {
        if (Config::get('simplecurl.cache_only')) return null;

        if ($this->verbose)
            $this->io->text("post_form: url=$url");
        ...
        $this->io->comment("post_form($url,$cache) size=" . strlen($body));
        $this->sleep_between_calls();

        if (is_array($options) && isset($options['headers'])) {
            return array($header, $body, $options['headers']);
        } else
            return $body;
    }
    ...
    {
        if ($this->verbose)
            $this->io->text(" unpack($unpack_folder,$file)");

        $ext = pathinfo($file, PATHINFO_EXTENSION);
        if ($this->verbose)
            $this->io->text(" UNPACK ($ext) $unpack_folder $file...");
        if ('zip' == $ext) {
        ...
        if (isset($cmd)) {
            if ($this->verbose)
                $this->io->text(" Shell command >> $cmd");
            $output = system($cmd, $status);
            if ($this->verbose)
                $this->io->text(" UNPACK output >>> $output");
        }
    }
