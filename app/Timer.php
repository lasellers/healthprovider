...
class Timer {
	...
	public function dump() {
		// $this->stop();
		$t = $this->get();
		$this->io->writeln( sprintf( "%8.2f seconds %s", $t, ( $this->running ? 'Running' : 'Stopped' ) ) );
		$this->io->writeln( " start {$this->start_time} end {$this->end_time}" );
	}
	...
	public function print_pps_time() {
		...
		// if (\App\CLI::is_cli())
		$this->io->writeln( "# $t seconds ($s2) ($s1 pages per second)" );
		// else
		//   $this->io->writeln("<span><b>$t</b> seconds ($s2) (<b>$s1</b> pages per second)</span>");
	}
	...
	public function print_elapsed_time() {
		...
		$s2  = $this->get_elapsed_time();
		//if (\App\CLI::is_cli())
		$this->io->writeln( "# $t seconds ($s2)" );
		//else
		//     $this->io->writeln("<span><b>$t</b> seconds ($s2)</span>");
	}
	...
	public function print_memory_used() {
		$this->io->writeln( $this->memory_used() );
	}
